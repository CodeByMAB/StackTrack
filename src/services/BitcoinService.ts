 ...
 * Using Bitcoin Treasury API: https://github.com/block/bitcoin-treasury
 */
export const BitcoinService = {
  /**
   * Get the current Bitcoin price in USD
   * Uses Bitcoin Treasury API with caching
   */
  async getBitcoinPrice(): Promise<BitcoinPrice> {
    try {
      ...
      // Fetch fresh data from Bitcoin Treasury API
      const response = await fetch('https://api.bitcoin-treasury.org/v1/price');
